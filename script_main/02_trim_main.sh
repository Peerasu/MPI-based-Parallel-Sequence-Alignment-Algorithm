...
threads=(16 16 16)                                      # Thread for trimmomatic

############################################################################## 


JOB_NAME_PREFIX="Trim"
TEMPLATE="$HOME/script/02_trim_sub.sh"
...
do
    ...
    JOB_SCRIPT="$HOME/script/02_trim_sub_${name}.sh"
    ...
    echo "Submitted job: $JOB_NAME with memory=${memory}gb, time=${time}, ppn=${ppn}, # Thread=${thread}"
